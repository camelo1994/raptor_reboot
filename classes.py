...
class menu:  # (settings_file,profiles_file,crosshair_file,bgm_file):
    ...
    def save_data(self):
        ...
        aux = 'UPDATE settings SET' + ' bgm_volume=' + str(self.bgm_volume) + ',' + 'last_player=' + str(
            self.profileindex)
...
class anim_rect:
    ...
    def update_rect(self):
        self.pos[0] += self.dx
        self.pos[1] += self.dy
        if self.pos[0] > sresH + 100 or self.pos[0] + self.w < -100 or self.pos[1] < -100 or self.pos[
            1] - self.h > sresV + 100:
            return True
        ...
        if pygame.time.get_ticks() - self.clock >= self.cooldown:
            ...
            render.projectiles.append(Projectile(self.projectile_image, pos[0] - 27, pos[1], 0, self.projectile_speedV,
                                                 anim=Animation(self.anim,
                                                                anim_rect(pos[0] - 22, pos[1] + 20, 0,
                                                                          self.projectile_speedV,
                                                                          8, 20), True), friendly=True,
                                                 damage=self.damage,
                                                 origin=self.name))
            n = len(render.sprites)
        ...
        if pygame.time.get_ticks() - self.clock >= self.cooldown:
            ...
            if a > 5:
                render.projectiles.append(
                    Projectile(self.projectile_image_inv, pos[0] + 15, pos[1], 0, self.projectile_speedV,
                               damage=self.damage,
                               friendly=True, origin=self.name + 'R'))
            else:
                render.projectiles.append(
                    Projectile(self.projectile_image, pos[0] + 15, pos[1], 0, self.projectile_speedV,
                               damage=self.damage,
                               friendly=True, origin=self.name + 'R'))
            if b > 5:
                render.projectiles.append(
                    Projectile(self.projectile_image_inv, pos[0] - 20, pos[1], 0, self.projectile_speedV,
                               damage=self.damage,
                               friendly=True, origin=self.name + 'L'))
            else:
                render.projectiles.append(
                    Projectile(self.projectile_image, pos[0] - 20, pos[1], 0, self.projectile_speedV,
                               damage=self.damage,
                               friendly=True, origin=self.name + 'L'))
    ...
    def fire(self, pos):
        if pygame.time.get_ticks() - self.clock >= self.cooldown:
            ...
            if self.has_target:
                ...
                pass #play_sound('laser_turret_prefire')
